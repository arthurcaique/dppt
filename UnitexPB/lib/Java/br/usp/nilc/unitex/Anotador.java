		...
		try{
			...
			while (linha != null){
				...
				for (int i =0; i < tamanho; i++){
					...
					// Se não for um caracter do alfabeto
					} else {
						// Se a palavra não for vazia
						if (palavra.length() > 0){
							...
							if (entrada.size() > 0){
								...
								out.println("<BR>");
								out.flush();
							// Se a palavra NAO estiver no dicionário
							} else {
								out.println(palavra + "<BR>");
								out.flush();
							}
							// Limpa a palavra para pegar a próxima
							palavra = "";
